...
namespace Winform.Dialogs
{
    public partial class ItemDialog : Form
    {
        ...
        private void CalSumPrice()
        {
            if (_isEditMode)
            {
                foreach (ItemRelation relation in _relations)
                    relation.Child = _unitOfWork.ItemRepos.GetBy(relation.ChildId);
                var item = new Item();
                var sum = item.GetSumPrice(_relations);
                this.numberUnitPrice.Value = sum * 90 / 100;
            }
            else
            {
                foreach (ItemRelation relation in _relations)
        ...
        {
            if (_selectedIndex < 0 || _selectedRelation == null)
            {
                MessageBox.Show("Không có chi tiết được chọn", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
